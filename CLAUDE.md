...
**이 구조는 엔진에만 남아 있고 화면에는 드러나지 않는다.** 음향 화면 첫 탭은
취향 프리셋만 이름으로 나열한 평면 목록이다. 기기 보정은 연결된 기기를 보고
자동으로 붙고, 환경 보정은 추가 효과 탭으로 옮겼다. 고를 것을 하나로 줄이려는
선택이다. 층 자체를 없앤 것이 아니므로 공유는 여전히 취향 층만 오간다.
...
lib/
  ...
  ui/
    ...
    effects_screen.dart    음향 효과(평면 목록) / 추가 효과 / 공유
    ...
    widgets/settings_list.dart    설정과 추가 효과가 함께 쓰는 목록 문법
    widgets/gesture_layer.dart    설정대로 한 손가락 조작을 받는 층
    gesture_settings_screen.dart  드래그·탭 제스쳐 배치
android/
...
Capriccio 6.5.2를 기기에 깔아 화면을 하나씩 찍어 대조했다. App Store의 마케팅
스크린샷은 믿을 것이 못 된다. 거기에는 재생 컨트롤 패널이 닫힌 상태로만 나와서
그 존재를 한참 몰랐다. 화면을 고칠 일이 있으면 실제 앱을 다시 찍어 보는 것이
빠르다.

되돌리면 안 되는 것 몇 가지가 있다.

**하단 탭바를 두지 않는다.** 재생 화면이 곧 첫 화면이고 라이브러리, 검색,
설정은 그 위에 쌓는다. 한 번 탭 넷으로 만들었다가 걷어냈다. 음악 플레이어는
손이 자주 가는 것이 재생 조작이라 화면 아래쪽을 그쪽에 내주는 편이 낫고,
자주 안 쓰는 설정은 위로 올리는 편이 낫다는 의견을 받았다.

그래서 하단은 `[폴더] 이전 재생 다음 [재생목록]`이고 상단은 큐 위치, 컨트롤
패널 토글, 검색, 설정이다. 음향은 설정 안으로 들어갔다.

**앨범아트가 제스쳐를 받는 자리다.** 남는 세로 공간을 전부 자켓에 준다. 컨트롤
패널을 펼쳐도 자켓을 밀어내지 않고 그 위에 유리로 얹는다. 자켓이 작아지면
제스쳐를 받을 면적도 같이 줄어든다.
...
**재생 컨트롤은 앨범아트 바로 아래 패널이다.** 상단 바의 둥근 버튼으로 여닫고,
윗줄은 `«10" ‹5" [A-B] 5"› 10"»`, 아랫줄은 `⊖ 속도 ⊕ ┃ ⊖ 피치 ⊕`다. 값을
바꾸러 다른 화면으로 가지 않는다. 속도 아이콘을 누르면 연동과 고정이 바뀌고,
피치 아이콘을 누르면 슬라이더와 숫자칸이 있는 연습 화면이 열린다.

**화면 순서를 지킨다.** 상단바 → 앨범아트 → 컨트롤 패널 → 제목 → 아티스트-앨범
→ 진행바 → 지난 시간·전체 길이 → 전송 행. 제목과 전송을 카드로 묶지 않는다.
한 번 묶어봤는데 Capriccio는 그렇게 하지 않는다.

**색이 역할을 나눈다.** 아티스트-앨범 줄과 지난 시간은 accent, 제목과 전체
길이는 흰색. 바뀐 값(속도, 피치)도 accent로 쓴다.

**효과 화면 첫 탭은 평면 목록이다.** `기본 효과 | 커스텀 효과` 알약 아래에
이름만 한 줄씩 놓고 지금 걸린 것을 accent로 쓴다. 체크 표시는 없다. 세 층으로
나눠 보여주던 화면을 갈아엎은 것이라 되돌리기 쉬운데, 그러면 첫 화면에서 고를
것이 셋으로 늘어난다.

탭은 알약 세그먼트가 아니라 아이콘이 붙은 탭 세 개고, 선택된 탭만 위쪽이 둥근
밝은 배경이며 아래에 가로선이 지난다.

**설정과 추가 효과는 같은 목록 문법을 쓴다.** `lib/ui/widgets/settings_list.dart`
에 있다. 섹션 라벨은 accent에 밑줄, 행은 굵은 제목과 회색 설명, 오른쪽에 accent
값과 꺾쇠, 누르면 목록 대화상자. 항목 이름도 그쪽 것을 그대로 쓴다. 탐색 시간 1,
탐색 시간 2, 속도 조절 단위, 피치 조절 단위. 기본값도 5초, 10초, 0.05x로 같다.

**미니 플레이어는 화면 폭을 다 쓴다.** 떠 있는 카드가 아니다. 위쪽 가장자리에
진행선, 제목은 흰색, 아티스트-앨범은 accent, 오른쪽에 이전·재생·다음.

**제스쳐는 전부 설정에서 바꾼다.** `lib/data/models/gesture_settings.dart`에
배치가 있고 `lib/ui/widgets/gesture_layer.dart`가 받는다. 쓸기(튕기기)와
끌기(붙인 채 움직이기)를 가르는데, 같은 축에 둘 다 걸면 어느 쪽인지 알 수
없으므로 쓸기를 켜두면 그 축의 끌기는 시작하지 않는다.

세로 기본값은 Capriccio와 반대다. 위로 쓸면 뒤로, 아래로 쓸면 앞으로. 조그셔틀
감각에 가깝다는 의견을 따랐다.

**재생 컨트롤 패널은 접힌 상태가 기본이다.** 늘 펼쳐두면 피치와 구간 반복을
스치듯 눌러 잘못 걸린다. 앨범아트를 길게 누르거나 상단 바 버튼으로 연다.

**연습·음향·EQ는 모달이 아니라 쌓는 화면이다.** 이게 제일 되돌리기
...
미니 플레이어는 재생 화면 위에 무언가 쌓였을 때만 나온다. 몇 겹 쌓였는지는
`NavigatorObserver`가 세어 `navDepthProvider`에 넣는다. 라우트 전환은 빌드
도중에 일어나므로 그 값은 다음 프레임에 쓴다. 시트와 대화상자는 세지 않는다.

미니 플레이어는 불투명하고 `SafeArea`로 시스템 바를 피한다. Scaffold는
`bottomNavigationBar`에 아래 여백을 넣어주지 않아서 직접 피해야 한다. 반투명으로
두면 뒤의 목록이 그대로 비쳐 글자가 겹친다.
...
앱을 껐다 켜면 듣던 곡이 그 위치에 다시 걸린다. **소리는 내지 않는다.**
이어폰을 안 꽂은 채 앱을 열었을 때 스피커로 터지는 것을 막으려는 것이다.

큐는 곡 id만 `KeyValues` 테이블에 담는다. 라이브러리에서 곡이 지워졌으면
꺼낼 때 걸러낸다. 위치는 250ms마다 바뀌는데 그때마다 쓰면 재생 내내 DB에
쓰게 되므로 5초에 한 번만 쓴다. 곡이 바뀌거나 멈출 때는 바로 쓴다.

되살릴 때 파일을 열지 않으므로 엔진에는 아무것도 없다. 재생 버튼을 누르는
시점에 `resume()`이 파일을 열고 저장해둔 자리로 옮긴다.
