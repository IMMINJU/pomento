...
**음향 화면 첫 탭은 평면 목록이다.** Capriccio의 Sound Effects와 같은 모양으로,
`프리셋 | 사용자 효과` 알약 아래에 이름만 한 줄씩 놓고 지금 걸린 것을 accent로
표시한다. 세 층으로 나눠 보여주던 화면을 갈아엎은 것이라 되돌리기 쉬운데,
그러면 첫 화면에서 고를 것이 셋으로 늘어난다.
