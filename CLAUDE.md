...
lib/
  ...
  data/
    db/database.dart       drift 스키마 (v2. 마크 테이블이 v2에서 붙었다)
    models/                eq_curve, preset, tempo, app_settings, gesture_settings, mark
    repo/                  library, preset, settings, mark
  ...
  ui/
    ...
    widgets/
      ...
      mark_parts.dart      마크 칩, 칩 줄, 마크 목록 행
...
눌러 잘못 걸린다. 패널 안은 `속도 · 구간 · 마크` 세그먼트로 내용을 바꾼다. 줄을
더하면 자켓이 또 준다.

**마크에 이름을 달지 않는다.** 듣다가 찍는 순간 키보드가 올라오면 그 대목을
놓친다. 시각이 이름 자리를 대신하고, 값이 걸린 마크만 값을 뒤에 붙인다. 점
마크는 동그라미, 구간 마크는 가로줄로 가른다. 처음부터 둘을 갈라 만들게 하면
찍는 순간에 판단이 하나 더 붙으므로 일단 점으로 찍고 나중에 올린다.

**마크는 진행바 안에서 그린다.** 점은 잉크 눈금, 구간은 그 구간만 옅게 칠한다.
새 선을 긋지 않으니 화면이 칸으로 안 잘리고, 파형이 필요 없어서 시각 값만 있으면
그려진다. 파형을 그리려면 `BufferingType.released` 밖에서 파일을 한 번 더 훑어야
하는데 7시간짜리에서는 비용이 크다.

**칩 줄은 접힌 상태에서만 나온다.** 패널의 마크 탭이 같은 일을 하므로 둘을 같이
두면 자켓만 두 번 깎인다. `+`는 스크롤 밖 오른쪽 끝에 고정이다. 같이 밀리면
마크가 많은 곡에서 찍을 길이 없어진다.

**연습본은 이름 대신 값이 가리킨다.** 배속과 센트가 그 묶음의 이름이다. 배속이
1.00이고 피치도 0이면 값이 가리키는 것이 없으므로 마크 수를 앞으로 올린다.
...
각 20% 언저리) + 얼룩(460px) + 알갱이(320px). 단색 하나면 균일해서 인쇄물이
아니라 화면으로 보인다. 색면 반지름은 화면보다 커야 그라디언트가 아니라 얼룩으로
읽힌다.

**따뜻한 색면만 세다**(`.32`, 반지름 120×90). 나머지 둘은 시안대로 `.19`와
`.17`이다. 시안의 `.20`에 `58%`로는 가장 진한 자리가 상단바에 가려지고 남는
몫이 바로 아래 앰비언트 판에 묻힌다. 종이 색면은 폭 전체에 10단계인데 판은
50단계가 넘어서, 옆에 판이 있으면 종이 쪽은 안 보인다.

**찬 회녹색은 올리지 말 것.** `#8AA6A0`은 셋 중 유일하게 `a*`가 -10.9다.
`.19`를 `.30`으로 올려봤더니 화면 오른쪽만 `a* -3`이 되어 회녹색이 아니라
청록으로 떴다. 밝기가 아쉬우면 색면이 아니라 `paper` 밑색을 건드려야 한다.

CSS `radial-gradient(88% 58% ...)`는 가로·세로 반지름을 따로 주는 **타원**인데
Flutter `RadialGradient.radius`는 **짧은 변** 하나에 대한 비라 그대로 옮기면
원이 된다. `GradientTransform`으로 세로를 늘려 타원으로 만든다.

**알갱이는 2.4%다.** 시안의 7%는 밝기가 17단계로 오르내려 실제 폰에서 화면이
지글거렸고 4.5%도 11단계라 보였다. 2.4%면 6단계다. 이 층이 빠지면 종이가
시안보다 1 L\* 밝게 앉는다. 곱하기라 평균 3단계 어둡게 만드는 몫이 사라져서다.
...
**색이 두 갈래다.** 자켓에서 온 색은 자켓 옆에서만 쓴다. **글자에는 안 쓴다.**
앰비언트 판, 진행선, 상단바 큐 밑줄, 미니 진행선 넷이 전부고 전부 채움이다.
`CoverTone.fill` 하나로만 나가고, 글자용 필드는 아예 없앴다. 한때 이 색이 배속
숫자와 알약 배경까지 다 칠해서 68군데에 걸려 있었는데, 곡이 바뀌면 화면 모양이
통째로 흔들렸다.

값과 설정은 **고정 강조색** `AppColors.accent`(`#364E86`)다. 배속·피치 숫자, 값
알약, A-B, 마크, 슬라이더, EQ 막대, 배지, 목록에서 지금 걸린 곡. 아이콘의
`brand`(`#26325C`)를 한 단 올린 값이다. 원래 남색은 종이 대비 11.5라 잉크(14.3)와
구별이 안 돼서 그냥 검정으로 읽힌다. 7.5여야 색으로 읽힌다.

**고르는 것은 잉크로 채운다.** 세그먼트, 프리셋 알약, `PaperPill`의 선택 상태.
고르는 것은 조작이지 재생이 아니다. Material `Switch`를 안 쓰는 이유도 이것이다.
누를 수 있다는 것은 색이 아니라 밑줄로 말한다.
